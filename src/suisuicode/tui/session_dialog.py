...
class InlineResumeWidget(Vertical, can_focus=True):
    ...
    def __init__(self, sessions: list, project_name: str = "", **kwargs) -> None:
    ...
    def _build_content(self) -> str:
        ...
        for i, meta in enumerate(self._filtered[:10]):
            title = getattr(meta, "title", None) or "(empty session)"
            if i == self._cursor:
                lines.append(f"[bold cyan]>[/] [bold]{title}[/]")
            else:
                lines.append(f"  {title}")

            parts = [_relative_time(meta.last_active)]
        ...
        if showing > 10:
            lines.append(f"  [dim]v {showing - 10} more session(s)[/]")
    ...
    def _refilter(self) -> None:
        if not self._search:
            ...
            self._filtered = [
                ...
                if s in (getattr(m, "title", None) or "").lower()
                or s in m.id.lower()
            ]
        self._cursor = 0
        self._refresh()
